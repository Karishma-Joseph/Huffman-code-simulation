...
void frontscreen()
{
	char a[9], b[47], c[18], d[13], e[4], f[33], g[35], h[21];
	strcpy_s(a, "SIR MVIT");
	strcpy_s(b, "DEPARTMENT OF COMPUTER SCIENCE AND ENGINEERING");
	strcpy_s(c, "A MINI PROJECT ON");
	strcpy_s(d, "HUFFMAN CODE");
	strcpy_s(e, "BY:");
	strcpy_s(f, "Animesh Mohan       (1MV15CS008)");
	strcpy_s(g, "Karishma Joseph       (1MV15CS047)");
	strcpy_s(h, "PRESS ENTER TO START");
...
int main(int argc, char** argv)
{
	...
	while (1) {
		...
		current->parent = NULL;
		current->next = NULL;
		strcpy_s(current->name, smallest1->name);
		strcat_s(current->name, smallest2->name);
